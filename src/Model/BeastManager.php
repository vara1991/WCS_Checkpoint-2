...
class BeastManager extends AbstractManager
{
    ...
    public function selectOneById(int $id)
    {
        $statement = $this->pdo->prepare(
            ...
            movie.id as id_movie, movie.title as movie_title, planet.id as planet_id, planet.name as planet_name
            FROM ".self::TABLE."
    ...
    {
        $statement = $this->pdo->prepare(
            "INSERT INTO ".self::TABLE." (`name`, `picture`, `size`, `area`, `id_movie`, `id_planet`) VALUES (:name, :picture, :size, :area, :movie, :planet)"
        );
        $statement->bindValue('name', $beast['name'], \PDO::PARAM_STR);
        $statement->bindValue('picture', $beast['picture'], \PDO::PARAM_STR);
        $statement->bindValue('area', $beast['area'], \PDO::PARAM_STR);
